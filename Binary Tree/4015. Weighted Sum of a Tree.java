...
class Solution {
    public long weightedSum(int[] parent, int[] nums) {
        ...
        return rec(tree, nums, 0, h, 1);
    }

    public int ht(ArrayList<ArrayList<Integer>> tree, int n, int d){
        int h = d;
        for(int ele : tree.get(n)){
            h = Math.max(h, ht(tree, ele, d + 1));
        }
        return h;
    }

    public long rec(ArrayList<ArrayList<Integer>> tree,int[] nums, int n, int h, int d){
        long ans = (long) nums[n] * (h - d + 1);
        for(int ele : tree.get(n)){
            ans += rec(tree, nums , ele, h, d + 1);
        }
        return ans;
    }
